	...
	$(function(){
  	...
  	*/
		$('#add').click(function(){
			...
			//JSON데이타 형식 사용]
			/*
			$('#div').attr({'title':'Click Here','class':'bg','style':'width:100px;height:100px;text-align:center;line-height:100px'}).on('click',function(){
				$(this).html('JQUERY');
			});*/
			//prop()함수 사용]
			//메소드 체인]
			...
			$('#div').prop({'title':'Click Here','class':'bg','style':'width:100px;height:100px;text-align:center;line-height:100px'}).on('click',function(){
				$(this).html('JQUERY');
			});		
      	...
      	$('#read').click(function(){
    		...
    		console.log('checkbox의 checked속성(prop) : ',$(':checkbox').prop('checked'));//false
      		...
      		//자스에서는 class나 style는 키워드로 사용됨으로
    		//this.class나
    		//$("선택자").get(0).class사용 불가 
    		//고로 attr()함수나 prop()함수로 속성값을 읽어 온다
    		console.log('div의 class속성(get()):',$('#div').get(0).class);//undefinded
      	});
		...
		$('a:eq(0)').click(function(){
			...
			console.log('표기하지 않는 속성 href(get(0).href):',$(this).get(0).href);//빈문자열
			console.log('표기하지 않는 속성 href(this):',this.href);//빈문자열
		});

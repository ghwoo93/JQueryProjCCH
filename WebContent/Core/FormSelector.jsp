...
<script>
	$(function(){
		...
		*/	
		console.log('input 태그 선택자 : ',$('input').length);
		console.log('폼 선택자 : ',$(':input').length);
		console.log(':button 선택자 : ',$(':button').length);//<button>도 포함된
		/*
	    		...
	    		case "값 읽기":
	    			...
	    			*/
	    			$(':button').each(function(i){
	    				...
	    				console.log('type=text의 입력값:',$(':text').val());//하나밖에 없으니까 each() 적용 불필요
	    				//3]input type="password"
		    			// :password나 혹은 input[type=password]
	    				console.log('type=password의 갯수:',$(':password').length);
	    				console.log('type=password의 입력값:',$(':password').val());
		    			...
		    			console.log('1. :checkbox:checked 와 each()함수');
		    			$(':checkbox:checked').each(function(){
		    				console.log($(this).val());
		    			});
		    			console.log('2. :checkbox 와 filter()함수 와 each()함수');
		    			$(':checkbox').filter(':checked').each(function(){
		    				console.log($(this).val());
		    			});
		    			console.log('3. :checkbox 와 if로 체크된 것 판단 그리고 each()함수');
	    			...
	    			console.log("$('input[type=image]').val() : ",$('input[type=image]').val());//빈문자열
	    			console.log("$(':image').val() : ",$(':image').val());//빈문자열
	    			console.log("$(':image').prop('value') : ",$(':image').prop('value'));//빈문자열
	    			//8]input type="radio"
					//선택에 상관없이 무조건 첫번째 요소값만 읽어옴]$(":radio").val()
	    			console.log("$(':radio').val() : ",$(':radio').val());
	    			//체크된 것만 얻기:$(":radio:checked"):each걸 필요 없다(하나만 석택되니까)
	    			console.log("$(':radio:checked').val() : ",$(':radio:checked').val());
	    			//9]reset 및 submit버튼
	    			console.log("$(':reset').val() : ",$(':reset').val());
	    			//버튼 태그도 type=submit 고로 무조건 첫번째 만 가져온다
